...
const WeatherCard = ({ city }) => {
  const [weatherInfo, setweatherInfo] = useState(null); //state for weather info
  const [forecastInfo, setforecastInfo] = useState([]); //state for 5-day forecast info
  ...
  const fetchWeather = async (cityName) => {
    ...
    try {
      ...
      const daily = forecastJson.list.filter((entry) =>
        entry.dt_txt.includes("12:00:00") //only gets the info at this hour from the api response
      );
  ...
  if (loading) {
    return (
      <p className="text-center mt-6 dark:text-gray-100 text-neutral900">
  ...
  if (!weatherInfo || weatherInfo.cod !== 200) {
    return (
      <p className="text-center mt-6 dark:text-gray-100 text-black">
  ...
  return (
    <div className="flex flex-col lg:grid lg:grid-cols-2 items-center p-10">
      <div
        className="w-full h-[100%] max-w-md p-6 mt-4 flex flex-col justify-center items-center font-poppins 
      ...
      >
        ...
        <div className="flex items-center justify-between gap-8 text-sm mt-4">
          <div className="flex items-center justify-between gap-2 transition-all duration-300 hover:scale-105 ease-in shadow-xl rounded-xl">
            <span className="material-symbols-outlined cursor-pointer text-black dark:text-white">
              water_drop
            </span>
      ...
      {/* 5 day  */}
      <div
        className="w-full h-[100%] p-6 mt-4 flex flex-col justify-between items-center font-poppins 
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-5 gap-5 text-center">
          {forecastInfo.map((day, index) => (
            <div
              key={index}
              className=" flex sm:flex-col gap-2 items-center justify-between bg-yellow-500 dark:bg-neutral-800 rounded-lg p-4 transition-all duration-300 hover:scale-105 ease-in shadow-md"
            >
